###############################################################
#  HARDCORE VERSION of doitall
# Functions included here:
# GetAllhcCharData - Pulls down the top 200 of each class, verifies class is correct
...
# GethcClassCharData - 
# MakehcHome - This creates the home page
# GethcNonZon - Gets non amazon characters that have bolts or arrows equipped, creates charts and html page
# GetUniqueProjectiles()
# GethcBong()
# GethcDashers - Dashing Strike builds
# GethcChargers - Gets characters with 10+ points in charge or wearing templars, creates charts and html page
# GethcOffensiveAuraItemsEquipped - Get character with 2 or more aura granting items
# MakehcClassPages - Makes the html pages for each class
#
# GitHubSync - Syncs all to Github; This should always be called last
###############################################################
...
def GetAllhcCharData():
    ...
    classes = [
#        {"class_dir": "/barbarian", "what_class": "Barbarian", "api": "5/1", "pages": 0},
#        {"class_dir": "/druid", "what_class": "Druid", "api": "6/1", "pages": 0},
#        {"class_dir": "/amazon", "what_class": "Amazon", "api": "1/1", "pages": 0},
#        {"class_dir": "/assassin", "what_class": "Assassin", "api": "7/1", "pages": 0},
#        {"class_dir": "/necromancer", "what_class": "Necromancer", "api": "3/1", "pages": 0},
#        {"class_dir": "/paladin", "what_class": "Paladin", "api": "4/1", "pages": 0},
#        {"class_dir": "/sorceress", "what_class": "Sorceress", "api": "2/1", "pages": 0}
...
#        {"class_dir": "/ladder-all", "what_class": "ladder1k", "api": "0/5", "pages": 0}
    ...
    # Determine SC or HC ladder URL
    if sc_or_hc == "sc":
        base_ladder_url = base_ladder_url + "0/"
    elif sc_or_hc == "hc":
        base_ladder_url = base_ladder_url + "1/"

    def generate_report(base_ladder_url, class_dir, what_class, api, pages):
        # Append API value to the base ladder URL
        ladder_url = base_ladder_url + api
        save_dir = sc_or_hc + class_dir

        print(f"url is {ladder_url}")
        print(f"local folder is {save_dir}")
        
        # Ensure the directory exists
        os.makedirs(save_dir, exist_ok=True)

        # Make the initial API call to fetch ladder data
        ...
        for character in ladder_data["ladder"]:
            ...
            char_id = character.get("id", None)  # Use ID if available
            char_level = character.get("level", 0)  # Fetch character level
            
            # Skip characters below level 60
            if char_level < 60:
                print(f"Skipping {char_name} (Level {char_level})")
                continue  

            # Fallback to ensure unique file names
            ...
            print(f"Saved {char_name} (Level {char_level}) to {file_path}")

        print(f"url is {ladder_url}")
        print(f"local folder is {save_dir}")

    # Iterate over each class and generate the report
    for cls in classes:
        generate_report(base_ladder_url, cls["class_dir"], cls["what_class"], cls["api"], cls["pages"])

def GethcClassCharData(filter_mismatches=True):  # Add option to enable/disable filtering
    base_ladder_url = "https://beta.pathofdiablo.com/api/ladder/13/"
    char_url = "https://beta.pathofdiablo.com/api/characters/{char_name}/summary"
    sc_or_hc = "hc"
...
def MakehcHome():
    ...
    data_folder = "hc/ladder-all"
    html_output = """"""
    output_file = "hcall_mercenary_report.html"
    ...
    plt.savefig("pod-stats/charts/hcclass_distribution.png", dpi=300, bbox_inches='tight', transparent=True)
...
#    html_content = template.render(html_output=html_output)  # Pass sorted clusters to the template
    ...
    with open('pod-stats/hcHome.html', 'w') as file:
...
def GethcUniqueProjectiles():
            ...
            try:
                with open(file_path, 'r') as file:
                    ...
                    if 'SkillTabs' in char_data and 'Equipped' in char_data:
                        ...
                        skill_data["Dead"] = char_data.get("IsDead", 'Unknown')
    ...
    # Define skill columns (exclude non-skill columns)
    skill_columns = [col for col in df.columns if col not in ['Name', 'Class', 'Level', 'Dead', 'Skills', 'Equipment', 'Mercenary', 'MercenaryEquipment'] + search_tags]
...
#            'label': f"{cluster} make up {cluster_percentage:.2f}%  of Unique Projectiles in use <br>Most popular skills used by characters with them equipped:<br>" + "<br>".join([f"<img src='{icons_folder}/{skill}.png' alt='{skill}' width='20' height='20'> {skill} {percent:.2f}% ({int(avg*character_count)})" for skill, avg, percent in top_skills_with_avg_and_percent[cluster]]),  # Use top skills with average points and percentages as cluster label        'character_count': character_count,  # Add character count to the data
            ...
            'other_skills': other_skills_str,
            'other_skills_pie': other_skills_pie,
            'characters': [{'name': row.Name, 'level': row.Level, 'dead': row.Dead, 'skills': row.Skills, 'equipment': row.Equipment, 'mercenary': row.Mercenary, 'mercenary_equipment': row.MercenaryEquipment } for row in sorted_group.itertuples()],
            'top_equipment': top_equipment_str,  # Add top equipment to the data
            'equipment_counts': equipment_counts_str,
            'remaining_skills_with_icons': remaining_skills_str_with_icons,
            'remaining_skills_str2': remaining_skills_str2,  # Add remaining skills string for display without icons
...
#    skill_columns = [col for col in df.columns if col not in ['Name', 'Class', 'Level', 'Skills', 'Equipment'] + search_tags]
   ...
   # Create clusters based on item presence
    df["Cluster"] = df[search_tags].apply(lambda row: "The Iron Jang Bong" if row["The Iron Jang Bong"] == 1 else "Warpspear", axis=1)
...
activePopup = null;
    ...
    for cluster, group in df.groupby('Cluster'):
        ...
        # Calculate the percentages and sort equipment counts
        equipment_counts = {worn: {title: (count / character_count) * 100 for title, count in titles.items()} for worn, titles in equipment_counts.items()}
        top_equipment = {worn: sorted(titles.items(), key=lambda item: item[1], reverse=True)[:5] for worn, titles in equipment_counts.items()}

        # Format the top equipment counts for display with percentages
        top_equipment_str_list = []
        for worn, titles in top_equipment.items():
            titles_str = "<br>".join([f"{title} {percent:.2f}%" for title, percent in titles])
            top_equipment_str_list.append(f"<strong>{worn.capitalize()}</strong>: <br>{titles_str}")
        top_equipment_str = "<br>".join(top_equipment_str_list)

        # Sort equipment counts in descending order
        sorted_equipment_counts = {
            worn: dict(sorted(titles.items(), key=lambda item: item[1], reverse=True))
            for worn, titles in equipment_counts.items()
        }

        # Format the full equipment counts for display with line breaks and bold category names
        equipment_counts_str_list = []
        for worn, titles in sorted_equipment_counts.items():
            titles_str = ", ".join([f"{title} {percent:.2f}%" for title, percent in titles.items()])
            equipment_counts_str_list.append(f"<strong>{worn.capitalize()}</strong>: {titles_str}")
        equipment_counts_str = "<br>".join(equipment_counts_str_list)
...
#            'label': f"{cluster} users:<br>" + "<br>".join([f"<img src='{icons_folder}/{skill}.png' alt='{skill}' width='20' height='20'> {skill} {percent:.2f}% ({int(avg*character_count)})" for skill, avg, percent in top_skills_with_avg_and_percent[cluster]]),  # Use top skills with average points and percentages as cluster label        'character_count': character_count,  # Add character count to the data
            ...
            'other_skills': other_skills_str,
            'other_skills_pie': other_skills_pie,
            'characters': [{'name': row.Name, 'level': row.Level, 'dead': row.Dead, 'skills': row.Skills, 'equipment': row.Equipment, 'mercenary': row.Mercenary, 'mercenary_equipment': row.MercenaryEquipment, 'class': row.Class } for row in sorted_group.itertuples()],
            'top_equipment': top_equipment_str,  # Add top equipment to the data
            'equipment_counts': equipment_counts_str,
            'remaining_skills_with_icons': remaining_skills_str_with_icons,
            'remaining_skills_str2': remaining_skills_str2,  # Add remaining skills string for display without icons
...
#
def GethcOffensiveAuraItemsEquipped():
    icons_folder = "icons"
...
#                                worn_category = 'Weapon'
                            ...
                            if worn_category not in equipment_titles:
                                equipment_titles[worn_category] = {}
                            title_tag = f"{title} {worn_category}"  # Combine Title and Tag
                            if title_tag not in equipment_titles[worn_category]:
                                equipment_titles[worn_category][title_tag] = 0
...
#    df['Cluster_Label'] = df['Cluster'].map(cluster_labels)
    ...
    <body class="not-main">
        ...
        <h1>Hardcore characters with 2 Offensive Aura items equipped </h1>
            <div class="summary-container">
                <br>
...
def MakehcClassPages():
    classes = [
        {"data_folder": "hc/barbarian", "what_class": "Barbarian", "howmany_clusters": 5, "howmany_skills": 5},
        {"data_folder": "hc/druid", "what_class": "Druid", "howmany_clusters": 7, "howmany_skills": 5},
        {"data_folder": "hc/amazon", "what_class": "Amazon", "howmany_clusters": 4, "howmany_skills": 5},
        {"data_folder": "hc/assassin", "what_class": "Assassin", "howmany_clusters": 5, "howmany_skills": 5},
        {"data_folder": "hc/necromancer", "what_class": "Necromancer", "howmany_clusters": 5, "howmany_skills": 5},
        {"data_folder": "hc/paladin", "what_class": "Paladin", "howmany_clusters": 4, "howmany_skills": 5},
        {"data_folder": "hc/sorceress", "what_class": "Sorceress", "howmany_clusters": 14, "howmany_skills": 5}
    ]
    icons_folder = "icons"
